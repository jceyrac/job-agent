...
## Prompt 5 — Delete the dormant profiles (dead-code cleanup, run LAST)

```
Repo: /Users/jeanclaudevd/AI-Suite/job_agent

GOAL
Remove the dead WEB3_REMOTE and CH_HYBRID profile definitions and every stale
reference to them. The app already runs single-profile (UNIFIED_JC); these two
are defined but unused. This is pure dead-code removal — no runtime behavior
changes. Run this LAST, after Prompts 1–4.

This prompt removes two unused profile OBJECTS. It does NOT remove the profile
concept: the SearchProfile dataclass, the (job_id, profile_id) schema, the
profile_id parameter on any function, and the search_profiles table all stay.
That dimension is the seam multi-user/multi-search depends on — leave it intact.

INVESTIGATION FIRST
1. Read profiles.py. Locate:
   - The "DORMANT PROFILES" comment banner (around line 60).
   - WEB3_REMOTE definition (starts ~line 67).
   - The "# Profil Suisse hybride" header + CH_HYBRID definition (~line 108–150).
   - The UNIFIED_JC intro comment (~line 152) that mentions
     "WEB3_REMOTE + CH_HYBRID multi-profile approach".
   - The registry comment near the bottom (~line 337): "Only the active profile
     is surfaced. Dormant profiles stay defined above..."
2. Confirm references with:
     git grep -n "WEB3_REMOTE\|CH_HYBRID\|web3_remote\|ch_hybrid"
   Expected hits outside profiles.py:
   - tests/test_storage.py (~line 2346): imports WEB3_REMOTE, CH_HYBRID and
     asserts their ids — this test breaks once they're deleted.
   - tests/test_scorer_parsing.py (~line 432): a docstring mentioning
     "web3_remote compat" — cosmetic only; the test does not import them.
   - scrape.py (~line 135): a comment mentioning web3_remote — Prompt 2 already
     rewrites this comment; confirm the web3_remote mention is gone.

CHANGES TO profiles.py
- Delete the DORMANT PROFILES comment banner.
- Delete the entire WEB3_REMOTE definition.
- Delete the "# Profil Suisse hybride" header and the entire CH_HYBRID definition.
- Edit the UNIFIED_JC intro comment so it no longer references the deleted
  profiles. Replace the "Tests whether a single rich scoring_context can replace
  the WEB3_REMOTE + CH_HYBRID multi-profile approach" wording with a plain
  description, e.g.: "The single search profile the app manages. A rich
  scoring_context is the primary preference-modelling mechanism; pre_filter
  carries only hard exclusions."
- Edit the bottom registry comment: remove "Dormant profiles stay defined above"
  (nothing is defined above anymore). Keep ALL_PROFILES, ACTIVE_PROFILE,
  DEFAULT_PROFILE_ID, get_active_profile(), and the from_criteria() helper exactly
  as Prompt 1 left them.

CHANGES TO tests/
- tests/test_storage.py: read the test around lines 2344–2348. It imports and
  asserts WEB3_REMOTE.id / CH_HYBRID.id. Delete that test function entirely — it
  validated the dormant profiles, which no longer exist. Do NOT delete
  test_default_profile_id_is_unified_jc; that one is still valid.
- tests/test_scorer_parsing.py (~line 432): optional — update the docstring to
  drop the "web3_remote compat" wording (e.g. "empty banned/hybrid_ok lists").
  The test body is unaffected; do not change its logic.

DO NOT TOUCH
- ALL_PROFILES, ACTIVE_PROFILE, DEFAULT_PROFILE_ID, get_active_profile().
- The job_scores (job_id, profile_id) schema, profile_id in any signature, the
  search_profiles table, or any storage method.

DONE WHEN
- git grep -n "WEB3_REMOTE\|CH_HYBRID" returns nothing.
- git grep -n "web3_remote\|ch_hybrid" returns nothing in code (ignore any DB rows
  — this is a code grep, not a data check).
- python -c "import profiles; print(profiles.get_active_profile().id)" prints
  unified_jc.
- pytest runs green (the obsolete dormant-profile test is removed, not failing).
```
...
- [ ] After Prompt 5: `git grep -n "WEB3_REMOTE\|CH_HYBRID"` returns nothing,
      `get_active_profile().id == "unified_jc"`, and `pytest` is green (the
      obsolete dormant-profile test removed, not failing).
...
- The only change to the dormant `WEB3_REMOTE` / `CH_HYBRID` profiles is their
  deletion in Prompt 5. Do not otherwise restructure `profiles.py`, and do not
  remove the profile *concept* (dataclass, `profile_id` dimension, registry).
